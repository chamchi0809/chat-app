...
export interface IUserModel extends Model<IUser> {
    ...
    updateAllStatus: (status: user_status) => Promise<void>
    ...
    editDraftByRoomId: (currentLoggedUser: string, roomId: string, draft: string) => Promise<any>
    getDraftByRoomId: (currentLoggedUser: string, roomId: string) => Promise<string>
}
...
userSchema.statics.updateAllStatus = async function (status: user_status) {
    try {
        await this.updateMany({}, {status: status})
    } catch (error) {
        throw error;
    }
}
...
userSchema.statics.editDraftByRoomId = async function (currentLoggedUser: string, roomId: string, draft: string) {
    try {
        const user = await this.findOne({_id: currentLoggedUser});
        user.drafts[roomId] = draft;
...
userSchema.statics.getDraftByRoomId = async function (currentLoggedUser: string, roomId: string) {
    try {
        const user = await this.findOne({_id: currentLoggedUser});
        return user.drafts[roomId] || '';
    } catch (error) {
        throw error;
    }
}
