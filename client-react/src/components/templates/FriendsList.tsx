...
const FriendsList: React.FC = () => {
    ...
    const selectedTabFriends = useMemo<IUser[] | undefined>(() => {
        switch (selectedTab) {
            case 'ONLINE':
                return filteredFriends.filter ?
                    filteredFriends?.filter(user => user.status !== 'offline')
                    :
                    [];
            case 'ALL':
                return filteredFriends;
            case 'ADD FRIEND':
                return searchResult?.filter(user => friends?.find ?
                    !friends?.find(friend => friend._id === user._id) && user._id !== auth.user?._id
                    :
                    true
                );
        }
    ...
    return (
        <StyledFriendsList>
            <div className="topSection">
                ...
                <Button color={oc.white} colorScheme={'gray'} highlightOffset={2} active={selectedTab === 'ADD FRIEND'}
                        onClick={() => {
